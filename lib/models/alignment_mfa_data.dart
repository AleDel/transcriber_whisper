...
class Tier {
  ...
  static Tier fromMap(Map<String, dynamic> map) {
    ...
    final List<Entry> entries = entriesList.map((entry) => Entry.fromList(List<dynamic>.from(entry))).toList(); // Usamos el método fromMap de Entry
    return Tier(
      type: map['type'],
      entries: entries,
    );
...
class Entry {
  ...
  static Entry fromList(List<dynamic> list) {
    return Entry(
